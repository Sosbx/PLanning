...
class WidgetStack(list):
    ...
    def popLayout(self):
        ...
        DEBUG('pop layout %s %s' % (layout.metaObject().className(), layout
            .objectName()))
        return layout

    def popWidget(self):
        widget = list.pop(self)
        DEBUG('pop widget %s %s' % (widget.metaObject().className(), widget
            .objectName()))
    ...
    def topIsLayoutWidget(self):
        ...
        return isinstance(parent, QtWidgets.QWidget) and type(parent) not in (
            QtWidgets.QMainWindow, QtWidgets.QStackedWidget, QtWidgets.
            QToolBox, QtWidgets.QTabWidget, QtWidgets.QScrollArea,
            QtWidgets.QMdiArea, QtWidgets.QWizard, QtWidgets.QDockWidget)
...
class UIParser(object):
    ...
    def __init__(self, qtcore_module, qtgui_module, qtwidgets_module,
        creatorPolicy):
    ...
    def _setupObject(self, class_name, parent, branch, is_attribute=True,
        ...
        object_name = self.uniqueName(branch.attrib.get('name') or
            class_name[1:].lower())
        if parent is None:
            ctor_args = ()
            ctor_kwargs = {}
        ...
        obj = self.factory.createQtObject(class_name, object_name,
            ctor_args=ctor_args, ctor_kwargs=ctor_kwargs, is_attribute=
            is_attribute)
    ...
    def createWidget(self, elem):
        ...
        if isinstance(parent, (QtWidgets.QDockWidget, QtWidgets.QMdiArea,
            QtWidgets.QScrollArea, QtWidgets.QStackedWidget, QtWidgets.
            QToolBox, QtWidgets.QTabWidget, QtWidgets.QWizard)):
            parent = None
        ...
        if isinstance(widget, QtWidgets.QTreeView):
            ...
            self.handleHeaderView(elem, 'horizontalHeader', widget.
                horizontalHeader())
            self.handleHeaderView(elem, 'verticalHeader', widget.
                verticalHeader())
        ...
        if isinstance(topwidget, QtWidgets.QToolBox):
            ...
            if icon is not None:
                topwidget.addItem(widget, icon, self.wprops.getAttribute(
                    elem, 'label'))
            else:
                topwidget.addItem(widget, self.wprops.getAttribute(elem,
                    'label'))
            ...
            icon = self.wprops.getAttribute(elem, 'icon')
            if icon is not None:
                topwidget.addTab(widget, icon, self.wprops.getAttribute(
                    elem, 'title'))
            else:
                topwidget.addTab(widget, self.wprops.getAttribute(elem,
                    'title'))
            tooltip = self.wprops.getAttribute(elem, 'toolTip')
            if tooltip is not None:
        ...
        elif isinstance(topwidget, (QtWidgets.QDockWidget, QtWidgets.
            QScrollArea)):
    ...
    def createSpacer(self, elem):
        ...
        sizeType = self.wprops.getProperty(elem, 'sizeType', QtWidgets.
            QSizePolicy.Policy.Expanding)
        policy = QtWidgets.QSizePolicy.Policy.Minimum, sizeType
        if self.wprops.getProperty(elem, 'orientation'
            ) == QtCore.Qt.Orientation.Horizontal:
            policy = policy[1], policy[0]
        spacer = self.factory.createQtObject('QSpacerItem', self.uniqueName
            ('spacerItem'), ctor_args=size_args + policy, is_attribute=False)
        if self.stack.topIsLayout():
            lay = self.stack.peek()
            lp = elem.attrib['layout-position']
            if isinstance(lay, QtWidgets.QFormLayout):
    ...
    def createLayout(self, elem):
        ...
        spacing = self.wprops.getProperty(elem, 'spacing', self.defaults[
            'spacing'])
        ...
        self.stack.push(self._setupObject(classname, parent, elem,
    ...
    def configureLayout(self, elem, layout):
        if isinstance(layout, QtWidgets.QGridLayout):
            self.setArray(elem, 'columnminimumwidth', layout.
    ...
    def handleItem(self, elem):
        if self.stack.topIsLayout():
            ...
            if isinstance(w, QtWidgets.QComboBox):
                ...
                item = self.createWidgetItem('QListWidgetItem', elem, w.
                    item, self.item_nr)
                w.addItem(item)
                ...
                for prop in elem.findall('property'):
                    ...
                    if c_prop_name == 'text':
                        ...
                        PlatformHelper.apply_background_color(item, column,
                            c_prop)
                    elif c_prop_name == 'foreground':
                        PlatformHelper.apply_foreground_color(item, column,
                            c_prop)
                ...
                self.disableSorting(w)
                item = self.createWidgetItem('QTableWidgetItem', elem, w.
                    item, row, col)
    ...
    def createWidgetItem(self, item_type, elem, getter, *getter_args):
        ...
        item = self.factory.createQtObject(item_type, 'item', is_attribute=
            False)
        props = self.wprops
        ...
        if background:
            PlatformHelper.apply_background_color(item, background)
        foreground = props.getProperty(elem, 'foreground')
        if foreground:
            PlatformHelper.apply_foreground_color(item, foreground)
    ...
    def addHeader(self, elem):
        ...
        if isinstance(w, QtWidgets.QTreeWidget):
            ...
            background = props.getProperty(elem, 'background')
            if background:
                PlatformHelper.apply_background_color(w.headerItem(), col,
                    background)
            foreground = props.getProperty(elem, 'foreground')
            if foreground:
                PlatformHelper.apply_foreground_color(w.headerItem(), col,
                    foreground)
    ...
    def createAction(self, elem):
        self._setupObject('QAction', self.currentActionGroup or self.
            toplevelWidget, elem)

    def createActionGroup(self, elem):
        action_group = self._setupObject('QActionGroup', self.
            toplevelWidget, elem, parent_is_optional=False)
        self.currentActionGroup = action_group
        self.traverseWidgetTree(elem)
        self.currentActionGroup = None
    widgetTreeItemHandlers = {'widget': createWidget, 'addaction':
        addAction, 'layout': createLayout, 'spacer': createSpacer, 'item':
        handleItem, 'action': createAction, 'actiongroup':
        createActionGroup, 'column': addHeader, 'row': addHeader, 'zorder':
        setZOrder}
    ...
    def _handle_widget(self, el):
        ...
        DEBUG('toplevel widget is %s', self.toplevelWidget.metaObject().
            className())
    ...
    def _handle_connections(self, el):
        ...
        for conn in el:
            ...
            slot = self.factory.getSlot(name2object(conn.findtext(
                'receiver')), conn.findtext('slot').split('(')[0])
            if signal_args == '':
    ...
    def _handle_custom_widgets(self, el):
        ...
        for custom_widget in el:
            ...
            self.factory.addCustomWidget(classname, custom_widget.findtext(
                'extends') or 'QWidget', header2module(custom_widget.
                findtext('header')))
